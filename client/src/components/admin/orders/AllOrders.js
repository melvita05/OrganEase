      ...
      <img
        ...
        src={
          product.id.pImages?.length
            ? `${apiURL}/uploads/products/${product.id.pImages[0]}`
            : "/no-image.png"
        }
        alt="productImage"
      />
      <span>{product.id.pincode }</span>
    </span>
  );
})}
...
<td className="hover:bg-gray-200 p-2 text-center cursor-default">
  {order.status === "Not processed" && <span className="text-red-600">{order.status}</span>}
  {order.status === "Processing" && <span className="text-yellow-600">Under Scrutiny</span>}
  {order.status === "Shipped" && <span className="text-blue-600">Request Accepted</span>}
  {order.status === "Delivered" && <span className="text-green-600">Request Completed</span>}
  ...
  {order?.allProduct?.map((product, i) => {
    ...
    return (
      <span key={i} className="block">
        ...
        <td className="hover:bg-gray-200 p-2 text-center">{order?.user?.name || "N/A"}</td>
        <td className="hover:bg-gray-200 p-2 text-center">
